...
为了添加一个新的上下文菜单到您的Web视图。首先在`webview/context`字段下的`menus`添加一个新的入口。每个贡献需要一个`command`（同样是项目标题所来之处）和一个`when`(时句)。[时句](/9.%20参考/6.%20时句上下文.md)包括`webviewId == 'YOUR_WEBVIEW_VIEW_TYPE'`来确保上下文菜单只应用在您的扩展的Web视图
```json
"contributes": {
  "menus": {
    "webview/context": [
