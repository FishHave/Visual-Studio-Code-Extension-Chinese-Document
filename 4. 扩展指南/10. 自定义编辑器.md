...
如果用户之后使用我们的自定义编辑器重新打开相同的资源，我们将会紧接着用全新的`CustomDocument`重新执行一遍整个`openCustomDocument`,`resolveCustomEditor`。

### 只读自定义编辑器基础

下面许多章节仅适用于支持编辑的自定义编辑器，并且，虽然听起来很矛盾，许多自定义编辑器完全不需要编辑功能。以图像预览或者内存转储可视化渲染为例。这都可以通过使用自定义编辑器实现，但都不需要可编辑。这就是`CustomReadonlyEditorProvider`的由来。

`CustomReadonlyEditorProvider`允许您创建不支持编辑的自定义编辑器。它们仍然可以交互，但不支持撤销、保存的操作。实现一个只读自定义编辑器同样比实现完全可编辑编辑器更简单。

### 可编辑自定义编辑器基础

可编辑自定义编辑器允许您使用标准VS Code操作，比如撤销和重做、保存和热退出。这使您的可编辑自定义编辑器非常强大，但同样意味着正确实现比实现一个可编辑自定义文本彼岸机器或者只读自定义编辑器更复杂。

可编辑自定义编辑器由`CustomEditorProvider`实现。这个接口继承自`CustomReadonlyEditorProvider`，所以您需要和一系列特定编辑操作一同实现如`openCustomDocument`和`resolveCustomEditor`的基本操作。

#### 编辑

对一个可编辑自定义文档的修改通过编辑表达。编辑可以是从文本更改到图像旋转和整理列表的任何事。VS Code把编辑的功能完全留给了您的扩展，但VS Code需要知道编辑何时发生。编辑是VS Code在文档变动时如何标记文档，这相继启用自动保存和归档。

无论用户在任何时候在您的自定义编辑器中作出任何编辑，您的扩展一定自其`CustomEditorProvider`触发`onDidChangeCustomDocument`事件。`onDidChangeCustomDocument`事件可以依据您的自定义编辑器实现触发两个事件类型：`CustomDocumentContentChangeEvent`和`CustomDocumentEditEvent`。

##### CustomDocumentContentChangeEvent

`CustomDocumentContentChangeEvent`是最基本的编辑。它唯一的功能是告诉VS Code某文档已编辑。

当扩展自`onDidChangeCustomDocument`触发`CustomDocumentContentChangeEvent`时，VS Code会标记关联文档为已变动。此时，用户让文档变成无变动的唯一方法是要么保存，要么恢复。使用`CustomDocumentContentChangeEvent`编辑的自定义编辑器不支持撤销/重做。

##### CustomDocumentEditEvent

`CustomDocumentEditEvent`是允许撤销/重做的更复杂的编辑。您应该尽量实现使用`CustomDocumentEditEvent`的自定义编辑器，并仅在撤销/重做不可使用时使用`CustomDocumentContentChangeEvent`。

`CustomDocumentEditEvent`具有以下字段：
- `document` - 编辑所属的`CustomDocument`。
- `label` - 介绍编辑所属类型的可选文本（例如：“裁剪”、“插入”……）
- `undo` - 当编辑需要撤销时VS Code调用的函数。
- `redo` - 当编辑需要被重做时VS Code调用的函数。

当扩展触发从`onDidChangeCustomDocument`触发`CustomDocumentEditEvent`时，VS Code将标记关联文档为已变动。用户为了让文档无变动，可以保存或恢复文档，或者撤销/重做到文档最后的保存状态。

编辑器`undo`和`redo`方法在特定编辑需要被撤销或重做时由VS Code调用。VS Code保存内部的编辑栈，所以如果您的扩展触发了具有三个编辑的`onDidChangeCustomDocument`时，姑且叫做`a`、`b`、`c`：
...
要实现撤销/重做，您的扩展必须更新其关联自定义文档的内部状态和文档所关联的所有Web视图，这样才能同步文档的新状态。请记住单个资源可能会有多个Web视图。它们必须显示相同的文档数据。例如同个图像编辑器的多个实例必须一直显示相同的像素数据，但可能允许每个编辑器实例拥有自己的缩放登记和UI状态。

#### 保存
当用户保存自定义编辑器时，您的扩展负责将当前状态下保存的资源写入磁盘。您的自定义编辑器如何完成此操作很大程度取决于您的扩展的`CustomDocument`类型和您的扩展内部如何追踪编辑。

保存的第一步是获取写入到磁盘的数据流。通常的操作包括：

- 追踪资源状态，以快速将其序列化。

例如基本的图像编辑器可能会存储像素数据缓存。

- 重放自上次保存的编辑，来生成新文件

例如更高效率的图像编辑器可能会追踪上次保存以来的编辑，例如`裁剪`、`旋转`、`缩放`。保存时，它会应用这些编辑到文件上次的保存状态来生成新文件。

- 向`WebviewPanel`所属自定义编辑器查询文件数据以保存。

请牢记虽然自定义编辑器可以在不可见时保存。正因此，推荐您不要依赖`WebviewPanel`实现`save`。如果无法做到，您可以设置`WebviewPanelOptions.retainContextWhenHidden`，让Web视图在隐藏时也能保持活动。由于`retainContextWhenHidden`会产生大量的内存开销，所以请谨慎使用。

在获取资源数据后，您通常应该使用[工作空间FS API](/9.%20参考/1.%20VS%20Code%20API.md)来将其写入磁盘。FS API采用`UInt8Array`存储数据，可以写入二进制或文本文件。对于二进制文件数据，只需将二进制数据转换为`UInt8Array`。对于文本文件数据，使用`Buffer`转换字符串为`UInt8Array`：

```typescript
const writeData = Buffer.from('我的文本数据', 'utf8');
vscode.workspace.fs.writeFile(fileUri, writeData);
```

## 下一步
如果您想要了解更多VS Code扩展功能，试试这些主题：

- [扩展API](/1.%20概览.md) - 了解整个VS Code扩展API
- [扩展功能](/3.%20扩展功能/1.%20概览.md) - 看看扩展VS Code的其他方式。