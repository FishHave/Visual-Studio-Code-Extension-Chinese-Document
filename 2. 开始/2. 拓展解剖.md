...
- `name`与`publisher`:VS Code使用`<publisher>.<name>`作为拓展独一无二的标识符。例如，Hello World示例拥有标识符`vscode-samples.helloworld-sample`。VS Code使用标识符识别您的扩展
- `main`:扩展的进入点
...
## 扩展进入文件
扩展进入文件输出两个行数，`activate`与`deactivate`。`activate`在您注册的**启用事件**中执行。`deactivate`给您机会在您的扩展禁用之前清理。